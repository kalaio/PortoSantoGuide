            ...
            {!isSelectionEmpty &&
                comboBoxContext?.selectedItems?.items?.map((value) => (
                    <span key={value.id} className="flex items-center rounded-md bg-primary py-0.5 pr-1 pl-1.25 ring-1 ring-primary ring-inset">
                        {value.avatarUrl ? <Avatar size="xxs" alt={value.label} src={value.avatarUrl} /> : null}

                        <p className="ml-1.25 truncate text-sm whitespace-nowrap text-secondary select-none">{value?.label}</p>
